...
{
    ESP_LOGD(TAG, "data_received = %u", jsn_sr04t_raw_data.data_received);
    ESP_LOGD(TAG, "IS AN EROOR = %u", jsn_sr04t_raw_data.is_an_error);
    ESP_LOGD(TAG, "RAW = %d", jsn_sr04t_raw_data.raw);
    ESP_LOGD(TAG, "distance in cm = %f", jsn_sr04t_raw_data.distance_cm);
}

void jsn_sr04t_print_data(jsn_sr04t_data_t jsn_sr04t_data)
{
    ESP_LOGD(TAG, "data_received = %u", jsn_sr04t_data.data_received);
    ESP_LOGD(TAG, "IS AN EROOR = %u", jsn_sr04t_data.is_an_error);
    ESP_LOGI(TAG, "distance in cm = %f", jsn_sr04t_data.distance_cm);
}
...
esp_err_t init_JSN_SR04T(jsn_sr04t_config_t *jsn_sr04t_config)
{
    ...
    if(ESP_OK != ret)
    {
        ESP_LOGE(TAG, "%s(). ABORT. error configuring the trigger gpio pin", __FUNCTION__);
        goto err;
    }
    ...
    if(ESP_OK != ret)
    {
        ESP_LOGE(TAG, "%s(). ABORT. error configuring the echo gpio pin", __FUNCTION__);
        goto err;
    }
    ...
    if(jsn_sr04t_config->no_of_samples == 0)
    {
        ...
        ESP_LOGE(TAG, "%s(). ABORT. jsn_sr04t_config->nbr_of_samples cannot be 0", __FUNCTION__);
    }

    if(jsn_sr04t_config->is_init == true)
    {
        ret = ESP_ERR_INVALID_ARG;
        ESP_LOGE(TAG, "%s(). ABORT. already init'd", __FUNCTION__);
    }
    ...
    */
    ...
    if(ESP_OK != ret)
    {
        ESP_LOGE(TAG, "%s(). ABORT. error configuring the RMT configuration", __FUNCTION__);
        goto err;
    }
    ...
    if(ESP_OK != ret)
    {
        ESP_LOGE(TAG, "%s(). ABORT. error installing the RMT Driver", __FUNCTION__);
        goto err;
    }
...
esp_err_t raw_measeurement(jsn_sr04t_config_t *jsn_sr04t_config, jsn_sr04t_raw_data_t *jsn_sr04t_raw_data)
{
    ...
    if (items) 
    {
        ...
        if(jsn_sr04t_raw_data->distance_cm < minimum_detection_value_in_cm) 
        {
            ...
            ESP_LOGE(TAG, "%s(). ABORT. Out Of Range: distance_cm < %d (%f) ", __FUNCTION__,
...
esp_err_t measurement(jsn_sr04t_config_t *jsn_sr04t_config, jsn_sr04t_data_t *jsn_sr04t_data)
{
    ...
    if(count_errors > 0)
    {
        ...
        ret = ESP_ERR_INVALID_RESPONSE;
        ESP_LOGE(TAG, "%s(). Abort At least one measurement is incorrect", __FUNCTION__);
        goto err;
    }
